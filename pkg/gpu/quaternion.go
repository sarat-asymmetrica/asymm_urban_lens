// Package gpu implements GPU-accelerated quaternion operations
// Ported from Quaternion OS Level Zero Go (geometric_consciousness_imaging)
//
...
func SLERP(q0, q1 Quaternion, t float32) Quaternion {
	...
	scale0 := float32(math.Sin(float64((1-t)*theta))) / sinTheta
	scale1 := float32(math.Sin(float64(t*theta))) / sinTheta
...
func FromAxisAngle(axis [3]float32, angle float32) Quaternion {
	...
	return Quaternion{W: c, X: nx * s, Y: ny * s, Z: nz * s}
}

// RotateVector rotates 3D vector by quaternion: v' = q ⊗ v ⊗ q*
